...
4. Use DNS challenges in NPM to get Let's Encrypt certificates for our services to make the annoying "Insecure Connection" messages finally go away.
...
Most guides won't mention this step as they generally assume you're exposing ports on your router and your router can forward ports 80 & 443 traffic to whatever port Nginx Proxy Manager (NPM) is using. We're using Tailscale and by default all traffic from our client devices will come directly to the server on ports 80 & 443, so we have to change this and put NPM here instead.
...
**NOTE:** *These steps are intentionally vague. Every DNS Manager is going to be different and you'll have to interpret the steps and instructions here for your use case. Generally speaking, we're adding an A record to our DNS record and pointing that to the Tailscale IP of our Unraid server.*
